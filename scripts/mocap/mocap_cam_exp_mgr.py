...
class MoCapCamExpManager(Node):
    def __init__(self):
        super().__init__('mocap_cam_exp_mgr_node')
        ...
        self.declare_parameter('oakd_l_det_topic', rclpy.Parameter.Type.STRING)
        self.declare_parameter('oakd_r_det_topic', rclpy.Parameter.Type.STRING)
        ...
        self.oakd_l_det_topic = self.get_parameter('oakd_l_det_topic').get_parameter_value().string_value
        self.oakd_r_det_topic = self.get_parameter('oakd_r_det_topic').get_parameter_value().string_value
        ...
        self.oakd_l_publisher = self.create_publisher(Detection3DArray,self.oakd_l_det_topic,10)
        self.oakd_r_publisher = self.create_publisher(Detection3DArray,self.oakd_r_det_topic,10)
        ...
        self.results_str = "experiment,gt1_x,gt1_y,gt1_z,track1_x,track1_y,track1_z,track1_id,track1_conf,gt2_x,gt2_y,gt2_z,track2_x,track2_y,track2_z,track2_id,track2_conf,unmatched_trks,n_dets,n_trks,delta_t\n"
    ...
    def tracker_callback(self, msg):
        ...
        self.results_str += "%s,%s,%s,%s," % (self.config, self.gt1_msg.pose.position.x, self.gt1_msg.pose.position.y, self.gt1_msg.pose.position.z)
    ...
    def run_experiments(self):
        ...
        for mcap_file in self.mcap_files:
            ...
            with open(os.path.join(self.results_dir, self.config + "_results.csv"), "w") as outfile:
                outfile.write("experiment,gt1_x,gt1_y,gt1_z,track1_x,track1_y,track1_z,track1_id,track1_conf,gt2_x,gt2_y,gt2_z,track2_x,track2_y,track2_z,track2_id,track2_conf,unmatched_trks,n_dets,n_trks,delta_t\n")
            outfile.close()
            ...
            while self.reader.has_next():
                ...
                if topic=='/tf':
                ...
                elif topic in self.oakd_l_det_topic:
                    msg_type = get_message(typename(topic))
                    msg = deserialize_message(data, msg_type)
                    ...
                    if self.odom_rcvd:
                        ...
                        # Send the detection
                        self.oakd_r_publisher.publish(msg)
...
def main(args=None):
    ...
    mocap_cam_exp_mgr = MoCapCamExpManager()
    mocap_cam_exp_mgr.run_experiments()

    # Shut down the node
    mocap_cam_exp_mgr.destroy_node()
    rclpy.shutdown()
